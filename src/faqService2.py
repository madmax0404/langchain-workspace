...
app.add_middleware(
    CORSMiddleware , # 웹브라우저와 파이썬서버간에 통신을 허용하기위한 미들웨어 
    allow_origins=["*"],
...
# 세션id를 기반으로 대화기록을 가져오거나 생성하는 함수. 
def get_session_history(session_id:str):
    if(session_id not in store):
        store[session_id] = ChatMessageHistory()
    return store[session_id]

# 8. 입력 변환을 위한 RunnableLambda설정
...
# uvicorn './faqService.py':app --host '0.0.0.0' --port 8000 --reload
